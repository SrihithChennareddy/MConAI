...
def compute_metrics(eval_pred):
    ...
    try:
        if len(set(labels)) > 1:  # at least 2 classes required for ROC AUC
...
def main():
    ...
    # --- Train/test per dataset/language with K-Fold cross-validation ---
    k_folds = 5
    for lang in df['language'].unique():
        for ds in df[df['language']==lang]['dataset'].unique():
            ...
            avg_fold_metrics = {k: round(sum(d[k] if d[k] is not None else 0 for d in fold_metrics)/len(fold_metrics),2) 
    ...
    avg_metrics = eval_df[["accuracy","precision","recall","f1","auc"]].mean()
    ...
    # Format AUC nicely
    eval_df["auc"] = eval_df["auc"].apply(lambda x: round(x,2) if x is not None else "N/A")
